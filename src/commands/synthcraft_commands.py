...
class SynthCraftCommands(commands.Cog):
    ...
    async def servers(self, interaction: discord.Interaction, server_id: int = None, show_hidden: bool = False):
        ...
        display_rows: list = []
        for row in rows:
            if row["state"] == "TRIAL" and not (row["total_seconds"] or 0):
        ...
        for state, group in groupby(display_rows, key=lambda r: r["state"]):
            ...
            for row in state_rows:
                ...
                if secs:
                    parts.append(f"{secs / 60:,.1f} min audio")
                    parts.append(f"${cost:.2f}")
                    parts.append(f"{row['generations']:,} gen")
                if row.get("activated_by_name"):
                    parts.append(row["activated_by_name"])
                lines.append(" \u00b7 ".join(parts))
    ...
    # =========================================================================
    # /synthcraft set-state
    # =========================================================================

    @synthcraft_group.command(name="set-state")
    @owner_only()
    @app_commands.describe(
    ...
    async def set_state(self, interaction: discord.Interaction, license_id: int, state: str):
        """Set a SynthCraft license to any state (TRIAL/ACTIVE/GRACE/EXPIRED)."""
        await interaction.response.defer(ephemeral=True)

        row = await self.db.fetchrow(
            "SELECT id, server_name, state, tier, credit_remaining, server_ip, label "
            "FROM synthcraft_licenses WHERE id = $1",
            license_id,
        )
        ...
        if state == "TRIAL":
            ...
            after = "ACTIVE / standard (unlimited)"
            color = discord.Color.green()
        elif state == "GRACE":
            await self.db.execute(
                """UPDATE synthcraft_licenses
